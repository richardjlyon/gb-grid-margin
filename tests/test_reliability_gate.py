"""Source-trace gate: reliability_year.json must match an independent raw-CSV recompute."""
...
SERIES = Path("site/data/reliability_year.json")
...
pytestmark = pytest.mark.skipif(
    ...
    reason="reliability_year.json or embedded/fuelhh store not present in this checkout",
)
...
def test_series_matches_independent_recompute():
    ...
    # Sample across the rolling year: every ~200th joined half-hour.
    ...
    for k in keys[::200]:
        ...
        if i < 0 or i >= len(values):
            continue  # outside the rolling-year window
